...
- [ ] Record the resulting local boundary, including why no current command or schema change is justified before the shared contract.
- [ ] Hand the implementation boundary to `MGIT-CLI-002` without selecting a speculative manifest or command surface.
