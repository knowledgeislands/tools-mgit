...
- [Define a repository set](repository-sets.md) covers `.mgit-config.toml`, `register`, and `bootstrap`.
...
By default, `mgit` discovers repository roots beneath the current directory. Use `mgit register` when the set should be explicit, reproducible, or include repositories outside that tree.
