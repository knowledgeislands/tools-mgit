...
- **supported-runtimes [M, fail]** `` `RUNTIMES-1` `` requires `[ki-repo]` to declare a non-empty, duplicate-free `supported_runtimes` list of runtimes the bootstrap linkers recognise (`claude-code`, `codex`). An unknown name has no discovery path, and an omitted or malformed list makes the repo's supported surface ambiguous; each FAILs. Local `.ki-config.toml` read, offline-safe (beside vendor-integrity); like `VENDOR-1` it is detect-only — the intended support surface cannot be guessed, so there is no `conform.ts` twin. (standard: `[ki-repo]` table — `supported_runtimes`)
...
- **RUNTIMES-J1 [J]** — Runtime orientation split: when `supported_runtimes` names a runtime other than `claude-code`, root orientation should live in a literal `AGENTS.md` with `CLAUDE.md` `@AGENTS.md`-importing it, rather than restating orientation in both files or leaving `CLAUDE.md` as the only copy a non-Claude-Code runtime can't read. Judgment, not mechanical — a repo may have a legitimate reason to skip the split (e.g. it targets `codex` only nominally and has no real Codex sessions). (standard: `[ki-repo]` table — `supported_runtimes`)
